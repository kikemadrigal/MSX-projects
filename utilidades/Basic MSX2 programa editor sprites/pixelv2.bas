...
1 'Bucle principal'
    ...
    110 if st=1 then Y=Y-8:CB=CB-1:IF Y<0 THEN Y=0
    120 if st=3 then X=X+8:CA=CA+1:IF X>120 THEN X=120
    130 if st=5 then Y=Y+8:CB=CB+1:IF Y>120 THEN Y=120
    140 if st=7 then X=X-8:CA=CA-1:IF X<0 THEN X=0
    1 'Captura de teclas'
    150 K$=INKEY$
...
1 'Rutina pintar en board y pixel abajo derecha al pulsar espacio
    250 PSET(tx+(X/8),ty+(Y/8)),C:LINE(X,Y)-(X+7,Y+7),C,BF
251 RETURN
...
1 'Rutina copiar en page 1 el tile actual'
    ...
    310 color 6:PRESET(150,8):PRINT#1,"Saved 1 page":for i=0 to 3000:next
    320 PRESET(150,8):PRINT#1,"              "
330 return

1 'Rutina ver page 1'
    1 'Ponemos el sprite del cursor fuera de a vista'
    400 PUT SPRITE 0,(0,212),0,0
...
1 ' el draw pinta el subrayado de aspecto'
    500 PRESET(180,155):PRINT#1,"Appearance":DRAW"bm180,164c15r52"
...
1 'Rutina crer archivo con los datas'
    ...
    710 color 6:PRESET(150,8):PRINT#1,"file created":for i=0 to 3000:next
...
1 'Rutina cargar archivo con los datas'
    ...
    760 color 6:PRESET(150,8):PRINT#1,"uploaded file":for i=0 to 3000:next
