...
main_loop:
    ...
    ;Este sistema de input se queda esperando a que se pulse una tecla
    ;call input_system1
    call input_system2
...
mover_personaje_arriba:
    ...
    sub 2 ; incrementamos en 3 el valor

    ;Chequeamos que no se salga por arriba
    cp 0
    ;salto condicional, comprobamos si el bit 6 edl registro F (el de las banderas) está a 1
    ; si es 1 saltamos al main_loop
    jp z, main_loop
...
mover_personaje_abajo:
    ld a,(ix); obetenemos el valor actual de la posicion y
    add 2 ; incrementamos en 3 el valor

    ;Chequeamos que no se salga por abajo
    cp 192-16
    jp z, main_loop
...
mover_personaje_derecha:
    ...
    ;Chequeamos que no se salga por la derecha
    cp 256-16
    jp z, main_loop
...
mover_personaje_izquierda:
    ...
    ;Chequeamos que no se salga por la izquierda
    cp 0
    jp z, main_loop
