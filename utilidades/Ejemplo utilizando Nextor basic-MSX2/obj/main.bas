...
1' ec()=enemigo color
55 DIM ex(3),ey(3),ev(3),el(3),es(3),ep(3),ec(3),ee(3)
...
100 px=0:py=0:pw=8:ph=16:pv=4:pl=8:pj=0:pa=0:pd=3:pe=5
...
145 bo=0:bx=0:bw=24:bh=16:bz=0:bi=0:by=0:be=100:bv=8:bd=3:ba=0:bb=0
1 'Al pulsar el espacio disparamos'
146 strig(0) on:on strig gosub 10500
1 'se=sonido de música elegida'
150 se=0
...
1 '1 'elegimos otra música'
1 '410 se=0
1 '1 'reproducimos la múscia'
420 gosub 7000
...
1'chequeando contorno sprite personaje
    ...
    4020 if px>255 then put sprite 0,(0,212),,p1:gosub 13600:py=18*8:px=8:ms=ms+1:gosub 13900: gosub 6000:'gosub 12700
    1 'Chekeo pantalla'
    ...
    4070 if t0<26 then vpoke md+tx+(ty+1*mw),tw:line (px-8,py+8)-(px+8,py+16),14,bf : wc=wc-1:beep'gosub 6000:re=6:gosub 7000
    1 'Si es un bloque de muerte, morimos'
    4080 if t0=td then gosub 10400:beep
    ...
    4100 t3=vpeek(md+tx+((ty+1)*mw))
    1 'Tile de abajo' 
    4110 t5=vpeek(md+tx+((ty+2)*mw))
    1 'tile de la izquierda'
    4120 t7=vpeek(md+tx+((ty+1)*mw)) 
    ...
    4170 if pa=1 and py<po-16 then pl=-pl
...
1 'Debug'
    6100 preset (10,30):F$(0)="!tf "+str$(tf)+" t3 "+str$(t3)+" pv "+str$(pv)+" t7 "+str$(t7): Z=USR(60)
    1 'BASIC VERSION'
    1 '6100 preset (10,30):print #1,"en "en" bo "bo
6190 return
...
7000 F$(0)="music.bin" 
    1 '1 'función 31 apertura de fichero, P(2) segmento y P(3) dirección'
    7020 E=USR(31)
    7030 P(2)=7: P(3)=0:P(4)=&H80f:E=USR(33)
    7040 Z=USR(32)
    7050 P(0)=7:P(1)=&he000:Z=USR(59)
    7060 'P(0)=8:P(1)=&he009:Z=USR(59)
7090 return

1 'Reproducimos bloque de música'
    7100 P(0)=7:P(1)=&he009:Z=USR(59)
7190 return
...
1 'Rutina player muere'
    ...
    10405 py=0:px=0: put sprite 0,(px,py),,p1  
...
12800 if en<=0 then return 
    12810 for i=1 to en
        ...
        12830 if ex(i) mod 5=0 then ev(i)=-ev(i)
        12840 if ev(i)>0 then es(i)=es(i)+1 else es(i)=es        
        12850 put sprite ep(i),(ex(i),ey(i)),ec(i),es(i) 
        1 'Colisión del enemigo con el player'
        12860 if px < ex(i) + 16 and  px + 16 > ex(i) and py < ey(i) + 16 and 16 + py > ey(i) then gosub 10400
        ...
        12870 for w=1 to dn
            ...
            12880 if dx(w) < ex(i) + 16 and  dx(w) + 15 > ex(i) and dy(w) < ey(i) + 16 and 2 + dy(w) > ey(i) then ed=i:gosub 12600:dd=w:gosub 11600
        12890 next w
    12895 next i
12899 return
...
1 '-----------------------------------'
    ...
    13000 if ma=0 then F$(0)="tilemap.bin": Z=USR(31): P(2)=0: P(3)=&H8000: P(4)=&h4000: Z=USR(34): Z=USR(32):gosub 20000:gosub 13900
    13010 if ma=1 then F$(0)="tilemap2.bin": Z=USR(31): P(2)=0: P(3)=&H8000: P(4)=&h4000: Z=USR(34): Z=USR(32):gosub 21000
    13020 if ma=2 then F$(0)="tilemap3.bin": Z=USR(31): P(2)=0: P(3)=&H8000: P(4)=&h4000: Z=USR(34): Z=USR(32):gosub 22000
    13195 gosub 13300
13199 return
...
1 'Aquí vamos dibujando las pantallas de un mundo,pintamos en la VRAM page 0, los valores definidos en el array hasta la columna 32
    13300 cls:md=&h8000
        13310 for f=0 to 23-1
            13320 for c=0 to 200-1
                13330 tn= vpeek(md):md=md+1
                1 ' Lamentablemente tengo que poner +2 y que el nextorbasic solo está preparado para screen4'
                13340 tn=tn+2
                    13350 if tn >=0 and tn <32 then copy (tn*8,0*8)-((tn*8)+8,(0*8)+8),pg to (c*8,f*8),0,tpset
                    13360 if tn >=32 and tn <64 then copy ((tn-32)*8,1*8)-(((tn-32)*8)+8,(1*8)+8),pg to (c*8,f*8),0,tpset
                    13370 if tn >=64 and tn <96 then copy ((tn-64)*8,2*8)-(((tn-64)*8)+8,(2*8)+8),pg to (c*8,f*8),0,tpset
                    13380 if tn>=96 and tn <128 then copy ((tn-96)*8,3*8)-(((tn-96)*8)+8,(3*8)+8),pg to (c*8,f*8),0,tpset
                    13390 if tn>=128 and tn <160 then copy ((tn-128)*8,4*8)-(((tn-128)*8)+8,(4*8)+8),pg to (c*8,f*8),0,tpset
                    13400 if tn>=160 and tn <192 then copy ((tn-160)*8,5*8)-(((tn-160)*8)+8,(5*8)+8),pg to (c*8,f*8),0,tpset
                    13410 if tn>=192 and tn <224 then copy ((tn-192)*8,6*8)-(((tn-192)*8)+8,(6*8)+8),pg to (c*8,f*8),0,tpset
                    13420 if tn>=224 and tn <256 then copy ((tn-224)*8,7*8)-(((tn-224)*8)+8,(7*8)+8),pg to (c*8,f*8),0,tpset
            13430 next c
            13440 'md=&h8000:md=md+(f*mw)
    13450 next f 
    13460 md=&h8000
13570 return
...
1 'Mover mapa a la izquierda y pintar ultima columna
    ...
    13603 for i=0+ms to 32+ms
        ...
        13620 for f=3 to 23-1 
            ...
            13630 tn=vpeek(md+(31+mc)+f*mw):tn=tn+2
...
1'------------------------------------'
    14000 cls:preset (10,30):  F$(0)="!@@@@  @  @@@@  @ @@@@@ @  @   @": Z=USR(60)
    ...
    14070 gosub 7100: if inkey$="" then goto 14070
    1 'BASIC VERSION'
