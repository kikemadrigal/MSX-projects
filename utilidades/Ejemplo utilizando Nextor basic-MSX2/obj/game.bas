...
55DIMex(3),ey(3),ev(3),el(3),es(3),ep(3),ec(3),ee(3)
60mc=0:ma=0:ms=0:mu=0:mw=200
70al=0:tf=0:td=0:te=0:tw=0:time=0:t1=0:t3=0:t5=0:t7=0:wc=0
90pg=2:md=&h8000
100px=0:py=0:pw=8:ph=16:pv=4:pl=8:pj=0:pa=0:pd=3:pe=5
110p1=0:p2=1:p3=2:p4=3
120pp=0:ps=0
130dm=4:dn=0:dd=0:ds=6:dc=6:dp=0:dw=8:dh=2
140em=3:en=0:es=11
145bo=0:bx=0:bw=24:bh=16:bz=0:bi=0:by=0:be=100:bv=8:bd=3:ba=0:bb=0
146strig(0)on:onstriggosub10500
150se=0
160gosub7000
210gosub14000
220P(0)=5:E=USR(77)
...
4020ifpx>255thenputsprite0,(0,212),,p1:gosub13600:py=18*8:px=8:ms=ms+1:gosub13900:gosub6000:'gosub12700
4030ifpx<0thenpx=0
4040ifpy>180thengosub10400
4050tx=px/8+mc:ty=py/8:ifpx<=0thentx=0:ifpy<=0thenty=0
4060t0=vpeek(md+tx+((ty+1)*mw))
4070ift0<26thenvpokemd+tx+(ty+1*mw),tw:line(px-8,py+8)-(px+8,py+16),14,bf:wc=wc-1:beep'gosub6000:re=6:gosub7000
4080ift0=tdthengosub10400:beep
4090ift0=tethenmu=1
4100t3=vpeek(md+tx+((ty+1)*mw))
4110t5=vpeek(md+tx+((ty+2)*mw))
4120t7=vpeek(md+tx+((ty+1)*mw))
...
4170ifpa=1andpy<po-16thenpl=-pl
...
7000F$(0)="music.bin"
7020E=USR(31)
7030P(2)=7:P(3)=0:P(4)=&H80f:E=USR(33)
7040Z=USR(32)
7050P(0)=7:P(1)=&he000:Z=USR(59)
7060'P(0)=8:P(1)=&he009:Z=USR(59)
7090return
7100P(0)=7:P(1)=&he009:Z=USR(59)
7190return
10400ba=0:bb=240:putsprite9,(ba,bb),,18
10405py=0:px=0:putsprite0,(px,py),,p1
10410pe=pe-1
...
12810fori=1toen
12820ex(i)=ex(i)+ev(i)
12830ifex(i)mod5=0thenev(i)=-ev(i)
12840ifev(i)>0thenes(i)=es(i)+1elsees(i)=es
12850putspriteep(i),(ex(i),ey(i)),ec(i),es(i)
12860ifpx<ex(i)+16andpx+16>ex(i)andpy<ey(i)+16and16+py>ey(i)thengosub10400
12870forw=1todn
12880ifdx(w)<ex(i)+16anddx(w)+15>ex(i)anddy(w)<ey(i)+16and2+dy(w)>ey(i)thened=i:gosub12600:dd=w:gosub11600
12890nextw
12895nexti
12899return
...
12999return
13000ifma=0thenF$(0)="tilemap.bin":Z=USR(31):P(2)=0:P(3)=&H8000:P(4)=&h4000:Z=USR(34):Z=USR(32):gosub20000:gosub13900
13010ifma=1thenF$(0)="tilemap2.bin":Z=USR(31):P(2)=0:P(3)=&H8000:P(4)=&h4000:Z=USR(34):Z=USR(32):gosub21000
13020ifma=2thenF$(0)="tilemap3.bin":Z=USR(31):P(2)=0:P(3)=&H8000:P(4)=&h4000:Z=USR(34):Z=USR(32):gosub22000
13195gosub13300
13199return
13300cls:md=&h8000
13310forf=0to23-1
13320forc=0to200-1
13330tn=vpeek(md):md=md+1
13340tn=tn+2
13350iftn>=0andtn<32thencopy(tn*8,0*8)-((tn*8)+8,(0*8)+8),pgto(c*8,f*8),0,tpset
13360iftn>=32andtn<64thencopy((tn-32)*8,1*8)-(((tn-32)*8)+8,(1*8)+8),pgto(c*8,f*8),0,tpset
13370iftn>=64andtn<96thencopy((tn-64)*8,2*8)-(((tn-64)*8)+8,(2*8)+8),pgto(c*8,f*8),0,tpset
13380iftn>=96andtn<128thencopy((tn-96)*8,3*8)-(((tn-96)*8)+8,(3*8)+8),pgto(c*8,f*8),0,tpset
13390iftn>=128andtn<160thencopy((tn-128)*8,4*8)-(((tn-128)*8)+8,(4*8)+8),pgto(c*8,f*8),0,tpset
13400iftn>=160andtn<192thencopy((tn-160)*8,5*8)-(((tn-160)*8)+8,(5*8)+8),pgto(c*8,f*8),0,tpset
13410iftn>=192andtn<224thencopy((tn-192)*8,6*8)-(((tn-192)*8)+8,(6*8)+8),pgto(c*8,f*8),0,tpset
13420iftn>=224andtn<256thencopy((tn-224)*8,7*8)-(((tn-224)*8)+8,(7*8)+8),pgto(c*8,f*8),0,tpset
13430nextc
13440'md=&h8000:md=md+(f*mw)
13450nextf
13460md=&h8000
13570return
13600'ifmc=200-32thenreturn
13603fori=0+msto32+ms
13605mc=mc+1
13610copy(8,0)-(256,184),0to(0,0),0,pset
13620forf=3to23-1
13630tn=vpeek(md+(31+mc)+f*mw):tn=tn+2
...
14070gosub7100:ifinkey$=""thengoto14070
14090goto220
