...
start /wait ..\..\tools\sjasm\sjasm.exe   asm world0.asm
...
..\..\tools\Hex2bin\hex2bin -e com %proga%.ihx
...
rem for /R dsk/ %%a in (*.*) do (
    rem start /wait ..\..\tools\Disk-Manager\DISKMGR.exe -A -F -C disco.dsk "%%a")   
:Emulator
rem copy disco.dsk tools\BlueMSX
rem start /wait Tools/BlueMSX/blueMSX.exe -diska dsk
